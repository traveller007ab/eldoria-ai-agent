...
import { ShieldCheck, AlertTriangle, CheckCircle, Info, InfoIcon, Loader2, ExternalLink } from 'lucide-react';
        ...
        <div className="panel flex-grow flex flex-col overflow-hidden">
            <div className="p-4 border-b border-cyan-500/20 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold uppercase tracking-widest text-cyan-300">Compliance Audit</span>
                </div>
                {isAuditing && <Loader2 className="w-3 h-3 text-cyan-400 animate-spin" />}
            </div>
                    ...
                    {scanResults ? (
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                {scanResults.matches.map((match: any, i: number) => (
                                    <div key={i} className="p-3 bg-black/40 border border-cyan-500/10 rounded-lg group hover:border-cyan-500/30 transition-all">
                                        <div className="flex items-center justify-between mb-1">
                                            <div className="text-[9px] font-bold text-cyan-100 truncate flex-grow">"{match.segment}"</div>
                                            <span className="text-[8px] text-orange-400 ml-2 shrink-0">{match.score}% match</span>
                                        </div>
                                        <div className="flex items-center gap-2">
